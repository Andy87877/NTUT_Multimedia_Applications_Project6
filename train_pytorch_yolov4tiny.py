...
def load_darknet_weights(model, weights_path):
    ...
    with open(weights_path, 'rb') as f:
        ...
        print(f"  [OK] Successfully loaded {loaded_layers}/21 layers from {Path(weights_path).name}")
        return True
...
def save_darknet_weights(model, filename):
    ...
    print(f"  [OK] Successfully exported weights to compliant Darknet format -> {filename}")
...
class YoloDataset(Dataset):
    def __init__(self, obj_dir, img_size=416):
        ...
        self.image_paths = sorted(list(self.obj_dir.glob("*.jpg")) + list(self.obj_dir.glob("*.png")))
        print(f"  Found {len(self.image_paths)} images in {self.obj_dir.name}/")
...
class YoloLoss(nn.Module):
    ...
    def forward(self, preds, targets, device):
        ...
        for scale_idx, pred in enumerate(preds):
            ...
            if loss_mask.sum() > 0:
                loss_box += self.mse_box(pred_xy[loss_mask == 1.0], txy[loss_mask == 1.0]) * 4.0
                loss_box += self.mse_box(pred_wh[loss_mask == 1.0], twh[loss_mask == 1.0]) * 2.0
...
def verify_and_predict(weights_path):
    ...
    img_t = torch.from_numpy(img_in).permute(2, 0, 1).float().unsqueeze(0) / 255.0
    ...
    for scale_idx, out in enumerate([out1, out2]):
        ...
        out = out.view(1, 3, 5 + NUM_CLASSES, H_out, W_out).permute(0, 1, 3, 4, 2)
    ...
    for det in detections:
        ...
        print(f"  [DETECT] Detected: {CLASS_NAMES[class_id]} at [{x1},{y1},{x2},{y2}] score={score:.4f}")
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % 5 == 0 or epoch == 1:
            print(f"  Epoch {epoch:2d}/{EPOCHS} | Loss: {epoch_loss:.4f} (box:{epoch_box:.3f}, conf:{epoch_conf:.3f}, cls:{epoch_class:.3f})")
    ...
    src_cfg = PROJECT_ROOT / "yolov4-tiny-custom.cfg"
    ...
    # Copy obj.names
    src_names = PROJECT_ROOT / "obj.names"
